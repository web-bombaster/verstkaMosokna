...
// Сделать инициализацию при выборе нового варианта

let initVariant = function() {

    const variantsWrapper = document.querySelectorAll('.options-type__variants');

    variantsWrapper.forEach(function(element, index) {
        variantsWrapper[index].querySelector('.variants-item').classList.add('toggle');
        if (variantsWrapper[index].closest('.left')) {
            variantsWrapper[index].closest('.left').querySelector('.options-thumb__item').classList.add('toggle');
        };
        if (variantsWrapper[index].closest('.left')) {
            variantsWrapper[index].closest('.left').nextElementSibling.querySelector('.option-img__item').classList.add('toggle');
        };
        // console.log(variantsWrapper[index].parentElement.parentElement);
    });
...
tabsToggle('.cover .variants-item', '.cover__box');
tabsToggle('.left .variants-item', '.left-tabs__item');
tabsToggle('.left .variants-item', '.right-tabs__item');

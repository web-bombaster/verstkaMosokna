...
let initClassToggleItem = function(parentElement, childElement) {
    if (document.querySelector(parentElement)) {
        const variantsWrapper = document.querySelectorAll(parentElement);
        variantsWrapper.forEach(function(element, index) {
            let ttt = variantsWrapper[index].querySelectorAll(childElement);
            ttt.forEach(function(element, index) {
                ttt[index].classList.remove('toggle');
            });

            variantsWrapper[index].querySelector(childElement).classList.add('toggle');
        });
    };
};
